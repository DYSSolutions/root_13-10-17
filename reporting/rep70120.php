...
function print_Chart_of_Accounts()
{
	...
	$cols = array(2, 50, 182, 380, 440, 480);

	$headers1 = array(_('    Code'), _('                   '), _('                           '),_('      Centre'), _('      Rs'), _('       Rs'));
...
//    $rep->getFontSize(12);
    ...
    else
    {
        ...
        $rep->MultiCell(160, 20, "Cheque Ref:  ".$from_trans['cheque'] ,0, 'L', 0, 2, 330,155, true);
    $rep->MultiCell(160, 20, "Cheque Date:  ".sql2date($from_trans['cheque_date']) ,0, 'L', 0, 2, 330,165, true);
    }
    $rep->MultiCell(160, 20, "Journal Ref:  ". $from_trans["ref"] ,0, 'R', 0, 2, 400,155, true);
...
//    $k = 0;
     $res=get_gl_trans11($from_trans['type'],$from_trans['trans_no']);
        ...
        while ($myrow1 = db_fetch($res)) {
            ...
            $rep->TextCol(3, 4,	$myrow1['dimension_id'], -2);
            if($myrow1['amount'] >0)
            {
                $rep->TextCol(4, 5,	number_format2($myrow1['amount']), -2);
            }
            else
            {
                $rep->TextCol(5, 6,	number_format2($myrow1['amount']*-1), -2);
            } $rep->NewLine(1);
//            $rep->font('b');
...
//            $rep->font('b');
            $rep->setfontsize(19);
            $rep->MultiCell(220, 20, "".$trans_name ." "."Voucher" ,0, 'C', 0, 2, 190,70, true);
            $rep->setfontsize(8);
//            $rep->font('');
...
//        alt_table_row_color($k);
    ...
    $rep->MultiCell(350, 20,"Amount in Words:" ."  ".convert_number_to_words(abs($from_trans['amount'])) ." "."Only",1, 'L', 0, 2, 60,540, true);
    $rep->MultiCell(120, 20, number_format2(abs($from_trans['amount'])),1, 'C', 0, 2, 420,540, true);
